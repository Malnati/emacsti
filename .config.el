(when (member window-system '(ns darwin))  (use-package exec-path-from-shell     :config (progn    (exec-path-from-shell-initialize))))

(use-package expand-region :config (progn (global-set-key (kbd "C-=") 'er/expand-region)))

(use-package multiple-cursors :config (progn  (define-prefix-command 'mc)  (global-set-key (kbd "C-c m") 'mc)  (define-key mc  (kbd "a") 'mc/mark-all-like-this)  (define-key mc  (kbd "n") 'mc/mark-next-like-this)  (define-key mc  (kbd "p") 'mc/mark-previous-like-this)  (global-set-key (kbd "M-<mouse-1>") 'mc/add-cursor-on-click)))

(use-package hydra)

(use-package ivy  :after hydra)

(use-package avy :init  :bind ("M-s" . avy-goto-char))

(use-package which-key  :config (progn  (progn    

(use-package company  :config (progn  (progn    ;; default is 0.2    (setq company-minimum-prefix-length 1 company-idle-delay 0.0)    (add-hook 'after-init-hook 'global-company-mode)))

(use-package dap-mode)

(use-package treemacs)

(use-package treemacs-projectile  :after treemacs  :after lsp-mode)

(use-package nord-theme  :config (progn  (add-to-list 'custom-theme-load-path
...
(use-package company-box  :after company-mode  :after lsp-mode  :hook (company-mode . company-box-mode)  :config (progn  (progn    (setq company-box-icons-unknown 'fa_question_circle)    (setq company-box-icons-elisp
...
(use-package indent-guide  :config (progn  (indent-guide-global-mode)  (setq indent-guide-char "|"))

(use-package drag-stuff  :config (progn  (progn    (drag-stuff-global-mode 1)    (drag-stuff-define-keys)    (global-set-key [(meta up)]   'drag-stuff-up)    (global-set-key [(meta down)] 'drag-stuff-down)))

(use-package projectile  :config (progn  (progn    (projectile-mode +1)    (define-key projectile-mode-map (kbd "C-c p") 'projectile-command-map)    (setq projectile-sort-order 'recentf)))

(use-package yasnippet  :config (progn  (progn    (yas-reload-all)    (add-hook 'prog-mode-hook #'yas-minor-mode)    (yas-global-mode 1)))

(use-package yasnippet-snippets  :after yasnippet  :config (progn  (progn    (when (member window-system '(pc w32 ms-dos windows-nt cygwin))      (setq yas-snippet-dirs
...
(use-package flycheck  :config (progn  (flycheck-mode t)  (setq global-flycheck-mode t))

(use-package helm  :config (progn  (progn    (global-set-key (kbd "M-x") 'helm-M-x)    (global-set-key (kbd "C-x C-f") #'helm-find-files)))

(progn  (setq lsp-keymap-prefix "M-l")
  (use-package lsp-mode     :hook    ((js2-mode . lsp-mode)     (lsp-mode . lsp-enable-which-key-integration))  :commands lsp  :config (progn (progn
